...
{
    if(cursorIsActive) {
        voxel_t block = 0; // a solid block that is not outside (but outside value will be recalculated later)
...
{
    if(cursorIsActive) {
        voxel_t block = VOXEL_EMPTY; // an empty block that is not outside (but outside value will be recalculated later)
